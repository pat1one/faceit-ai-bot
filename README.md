...
#### ⚙️ Безопасность и лимиты запросов

Основные переменные окружения для защиты API и загрузки демок:

- `RATE_LIMIT_REQUESTS_PER_MINUTE` / `RATE_LIMIT_REQUESTS_PER_HOUR` — глобальные лимиты запросов на IP/пользователя.
- `RATE_LIMIT_BAN_ENABLED` — включить/выключить автобан за систематическое превышение лимитов (использует Redis).
- `RATE_LIMIT_BAN_THRESHOLD` — сколько превышений лимита за окно считается поводом для бана.
- `RATE_LIMIT_BAN_WINDOW_SECONDS` — окно в секундах, в течение которого считаются нарушения.
- `RATE_LIMIT_BAN_TTL_SECONDS` — время бана в секундах (по умолчанию 1 час).
- `MAX_DEMO_FILE_MB` — максимальный размер загружаемой демки в мегабайтах (по умолчанию 100 МБ).

Рекомедуемые значения для продакшена:

- `RATE_LIMIT_REQUESTS_PER_MINUTE=60`
- `RATE_LIMIT_REQUESTS_PER_HOUR=1200`
- `RATE_LIMIT_BAN_ENABLED=true`
- `RATE_LIMIT_BAN_THRESHOLD=10`
- `RATE_LIMIT_BAN_WINDOW_SECONDS=300`
- `RATE_LIMIT_BAN_TTL_SECONDS=3600`
